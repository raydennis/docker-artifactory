...
SYS_KEY_PDN_NODE_TRACKERURL="pdn.node.trackerUrl"
SYS_KEY_PDN_NODE_JOINKEY="pdn.node.joinKey"
SYS_KEY_PDN_NODE_SELFGRPCADDRESS="pdn.node.selfGrpcAddress"
SYS_KEY_PDN_NODE_SELFHTTPADDRESS="pdn.node.selfHttpAddress"
SYS_KEY_PDN_NODE_PORT="pdn.node.port"
SYS_KEY_PDN_NODE_HTTPPORT="pdn.node.httpPort"
SYS_KEY_PDN_NODE_HTTPBOUNCERPORT="pdn.node.httpBouncerPort"
...
XRAY_LABEL="Xray"
PDN_LABEL="Private Distribution Network"
...
MESSAGE_PDN_NODE_JOINKEY="PDN Join key value generated in the Artifactory node an used for establishing a connection between the Nodes to the Tracker.\n(You can copy the PDN Access Token Join Key from Admin > Security > Settings)"
PROMPT_PDN_NODE_JOINKEY="Join Key"
KEY_PDN_NODE_JOINKEY="$SYS_KEY_PDN_NODE_JOINKEY"
IS_SENSITIVE_PDN_NODE_JOINKEY="$FLAG_Y"
REGEX_PDN_NODE_JOINKEY="^[a-zA-Z0-9]{16,}\$"
ERROR_MESSAGE_PDN_NODE_JOINKEY="Invalid Join Key"

KEY_PDN_NODE_PORT="$SYS_KEY_PDN_NODE_PORT"
KEY_PDN_NODE_HTTPPORT="$SYS_KEY_PDN_NODE_HTTPPORT"

MESSAGE_PDN_NODE_HTTPBOUNCERPORT="The PDN http bouncer server secure port, used as a load balancer between the PDNs"
PROMPT_PDN_NODE_HTTPBOUNCERPORT="HttpBouncerPort"
KEY_PDN_NODE_HTTPBOUNCERPORT="$SYS_KEY_PDN_NODE_HTTPBOUNCERPORT"

MESSAGE_PDN_NODE_TRACKERURL(){
    echo -e "Artifactory URL on which the Tracker accepts secure gRPC connections"
}

PROMPT_PDN_NODE_TRACKERURL="PDN Node Tracker URL"
KEY_PDN_NODE_TRACKERURL="$SYS_KEY_PDN_NODE_TRACKERURL"
REGEX_PDN_NODE_TRACKERURL=".*:{0,}[0-9]{0,4}\$"
ERROR_MESSAGE_PDN_NODE_TRACKERURL="Invalid Pdn Node Tracker URL"

MESSAGE_PDN_NODE_SELFHTTPADDRESS(){
    echo -e "HTTP address to expose for external clients with the http port "
}

PROMPT_PDN_NODE_SELFHTTPADDRESS="PDN Node SelfHttpAddress"
KEY_PDN_NODE_SELFHTTPADDRESS="$SYS_KEY_PDN_NODE_SELFHTTPADDRESS"
REGEX_PDN_NODE_SELFHTTPADDRESS=".*:{0,}[0-9]{0,4}\$"
ERROR_MESSAGE_PDN_NODE_SELFHTTPADDRESS="Invalid Pdn Node SelfHttpAddress"

MESSAGE_PDN_NODE_SELFGRPCADDRESS(){
    echo -e "GRPC address to expose for other PDNs with the gRPC port"
}

PROMPT_PDN_NODE_SELFGRPCADDRESS="PDN Node SelfGrpcAddress"
KEY_PDN_NODE_SELFGRPCADDRESS="$SYS_KEY_PDN_NODE_SELFGRPCADDRESS"
REGEX_PDN_NODE_SELFGRPCADDRESS=".*:{0,}[0-9]{0,4}\$"
ERROR_MESSAGE_PDN_NODE_SELFGRPCADDRESS="Invalid Pdn Node SelfGrpcAddress"
...
POSTGRES_SLES_VER="13.4-1"
POSTGRES_RPM_VER="13.4-1"
POSTGRES_DEB_VER="13_13.4-1"
...
setPostgresVersion() {
    ...
    case "$PRODUCT_NAME" in
        $ARTIFACTORY_LABEL)
            ...
            JFROG_POSTGRES_VERSIONS=("9.6.11" "10.13" "12.3" "12.5" "13.2" "13.4")
        ;;
        $JFMC_LABEL)
            JFROG_POSTGRES_9X_VERSION="9-6-11v"
...
wrapper_setInstallerDefaults() {
    ...
    if [[ "${PRODUCT_NAME}" != "${PDN_LABEL}" ]]; then
...
getPdnNodeTrackerUrlIfAvailable() {
    if [ ! -z "$JF_PDN_NODE_TRACKERURL" ]; then
        return
    fi
    logDebug "Method ${FUNCNAME[0]} Caller:$(caller)"
    local productHome="$(_getDataDir)"
    if [ -z "${productHome}" ]; then
        logDebug "No key: [$KEY_DATA_DIR_LOCATION] found. Aborting PDN Node Tracker Url search"
        return
    fi
    
    local systemYaml="${productHome}/etc/${FILE_NAME_SYSTEM_YAML}"
    getYamlValue "$SYS_KEY_PDN_NODE_TRACKERURL" "${systemYaml}" "false"
    if [ ! -z "${YAML_VALUE}" ]; then
        logger "$PROMPT_PDN_NODE_TRACKERURL: [$SYS_KEY_PDN_NODE_TRACKERURL] found in system.yaml. Skipping prompt"
        JF_PDN_NODE_TRACKERURL="$YAML_VALUE"
        return
    fi
    logDebug "PDN NODE TRACKER URL not found"
}

getPdnNodeSelfGrpcAddressIfAvailable() {
    if [ ! -z "$JF_PDN_NODE_SELFGRPCADDRESS" ]; then
        return
    fi
    logDebug "Method ${FUNCNAME[0]} Caller:$(caller)"
    local productHome="$(_getDataDir)"
    ...
    local systemYaml="${productHome}/etc/${FILE_NAME_SYSTEM_YAML}"
    getYamlValue "$SYS_KEY_PDN_NODE_SELFGRPCADDRESS" "${systemYaml}" "false"
    if [ ! -z "${YAML_VALUE}" ]; then
        logger "$PROMPT_PDN_NODE_SELFGRPCADDRESS: [$SYS_KEY_PDN_NODE_SELFGRPCADDRESS] found in system.yaml. Skipping prompt"
        JF_PDN_NODE_SELFGRPCADDRESS="$YAML_VALUE"
        if [ ! -z "${JF_PDN_NODE_SELFGRPCADDRESS}" ]; then
            local pdnNodeGrpcPort=$(echo ${JF_PDN_NODE_SELFGRPCADDRESS} | awk -F":" '{print $2}')
            [ ! -z "${pdnNodeGrpcPort}" ] && setSystemValue "${SYS_KEY_PDN_NODE_PORT}" "${pdnNodeGrpcPort}" "$INSTALLER_YAML"
        fi
        return
    fi
...
getPdnNodeSelfHttpAddressIfAvailable() {
    if [ ! -z "$JF_PDN_NODE_SELFHTTPADDRESS" ]; then
        return
    fi
    logDebug "Method ${FUNCNAME[0]} Caller:$(caller)"
    local productHome="$(_getDataDir)"
    ...
    local systemYaml="${productHome}/etc/${FILE_NAME_SYSTEM_YAML}"
    getYamlValue "$SYS_KEY_PDN_NODE_SELFHTTPADDRESS" "${systemYaml}" "false"
    if [ ! -z "${YAML_VALUE}" ]; then
        logger "$PROMPT_PDN_NODE_SELFHTTPADDRESS: [$SYS_KEY_PDN_NODE_SELFHTTPADDRESS] found in system.yaml. Skipping prompt"
        JF_PDN_NODE_SELFHTTPADDRESS="$YAML_VALUE"
        if [ ! -z "${JF_PDN_NODE_SELFHTTPADDRESS}" ]; then
            local pdnNodeHttpPort=$(echo ${JF_PDN_NODE_SELFHTTPADDRESS} | awk -F":" '{print $2}')
            [ ! -z "${pdnNodeHttpPort}" ] && setSystemValue "${SYS_KEY_PDN_NODE_HTTPPORT}" "${pdnNodeHttpPort}" "$INSTALLER_YAML"
        fi
        return
    fi
...
_askForMandatoryInputs() {
    ...
    for key in "${mandatoryFields[@]}"; do
        case ${key} in
            ...
            ;;
            JF_PDN_NODE_TRACKERURL)
                getPdnNodeTrackerUrlIfAvailable
                if [ ! -z "$JF_PDN_NODE_TRACKERURL" ]; then
                    wrapper_setSystemValue "$KEY_PDN_NODE_TRACKERURL" "$JF_PDN_NODE_TRACKERURL"
                else
                    wrapper_askUser "PDN_NODE_TRACKERURL"
                fi
            ;;
            JF_PDN_NODE_SELFGRPCADDRESS)
                getPdnNodeSelfGrpcAddressIfAvailable
                if [ ! -z "$JF_PDN_NODE_SELFGRPCADDRESS" ]; then
                    wrapper_setSystemValue "$KEY_PDN_NODE_SELFGRPCADDRESS" "$JF_PDN_NODE_SELFGRPCADDRESS"
                else
                    wrapper_askUser "PDN_NODE_SELFGRPCADDRESS"
                fi
            ;;
            JF_PDN_NODE_SELFHTTPADDRESS)
                getPdnNodeSelfHttpAddressIfAvailable
                if [ ! -z "$JF_PDN_NODE_SELFHTTPADDRESS" ]; then
                    wrapper_setSystemValue "$KEY_PDN_NODE_SELFHTTPADDRESS" "$JF_PDN_NODE_SELFHTTPADDRESS"
                else
                    wrapper_askUser "PDN_NODE_SELFHTTPADDRESS"
                fi
            ;;
        esac
    done
...
wrapper_setPostgresVersion() {
    ...
    local systemYaml="${productHome}/etc/${FILE_NAME_SYSTEM_YAML}"
    local postgresVer=${JFROG_POSTGRES_13_4X_VERSION}
    if [[ "${TRIAL_FLOW}" == "${FLAG_Y}" ]]; then
...
_askPdnNodeJoinKey() {
    _updateFromSystemYaml "PDN_NODE_JOINKEY" "$KEY_PDN_NODE_JOINKEY"
    if [ -z "$YAML_VALUE" ]; then
        echo ""
        echo -n "Do you want to add PDN joinKey?[Y/n]: "
        read USER_CHOICE
        ...
        if [[ "${USER_CHOICE}" == "${FLAG_Y}" ]]; then
           unset USER_CHOICE
           wrapper_askUser "PDN_NODE_JOINKEY"
           getSystemValue "$SYS_KEY_PDN_NODE_JOINKEY" "NOT_SET" "false" "$INSTALLER_YAML"
           PDN_NODE_JOINKEY=${YAML_VALUE}
        fi
    else
        logger "Found $PROMPT_PDN_NODE_JOINKEY in system.yaml. Skipping prompt"
    fi
}

_askPdnNodeHttpBouncerPort() {
    _updateFromSystemYaml "PDN_NODE_HTTPBOUNCERPORT" "$KEY_PDN_NODE_HTTPBOUNCERPORT"
    if [ -z "$YAML_VALUE" ]; then
        echo ""
        echo -n "Do you want to change default PDN httpBouncerPort?[Y/n]: "
        read USER_CHOICE
        USER_CHOICE="$(echo "$USER_CHOICE" | tr '[:upper:]' '[:lower:]')"
        while [[ -z $USER_CHOICE || ! ${USER_CHOICE} == $FLAG_N && ! ${USER_CHOICE} == $FLAG_Y ]]; do
           warn "Invalid choice. Please retry"
        ...
        if [[ "${USER_CHOICE}" == "${FLAG_Y}" ]]; then
           unset USER_CHOICE
           wrapper_askUser "PDN_NODE_HTTPBOUNCERPORT"
           getSystemValue "$SYS_KEY_PDN_NODE_HTTPBOUNCERPORT" "NOT_SET" "false" "$INSTALLER_YAML"
           PDN_NODE_HTTPBOUNCERPORT=${YAML_VALUE}
        fi
    else
        logger "Found $PROMPT_PDN_NODE_HTTPBOUNCERPORT in system.yaml. Skipping prompt"
    fi
}
...
docker_showSuccessMessage() {
    logDebug "Method ${FUNCNAME[0]} Caller:$(caller)"
    if [[ "${PRODUCT_NAME}" == "${PDN_LABEL}" ]]; then
        docker_getSystemValue "$SYS_KEY_PDN_NODE_TRACKERURL" "$FLAG_NOT_APPLICABLE" "$SYSTEM_YAML_FILE"
...
docker_createSystemYaml() {
    ...
    if [ ! -z "${JF_PDN_NODE_TRACKERURL}" ]; then
        docker_setSystemValue "${SYS_KEY_PDN_NODE_TRACKERURL}" "${JF_PDN_NODE_TRACKERURL}" "${SYSTEM_YAML_FILE}"
    fi

    if [ ! -z "${JF_PDN_NODE_SELFGRPCADDRESS}" ]; then
        docker_setSystemValue "${SYS_KEY_PDN_NODE_SELFGRPCADDRESS}" "${JF_PDN_NODE_SELFGRPCADDRESS}" "${SYSTEM_YAML_FILE}"
    fi

    if [ ! -z "${JF_PDN_NODE_SELFHTTPADDRESS}" ]; then
        docker_setSystemValue "${SYS_KEY_PDN_NODE_SELFHTTPADDRESS}" "${JF_PDN_NODE_SELFHTTPADDRESS}" "${SYSTEM_YAML_FILE}"
    fi
    ...
    docker_installToSystemYaml "$KEY_PDN_NODE_TRACKERURL" "$FLAG_NOT_APPLICABLE" "$SYS_KEY_PDN_NODE_TRACKERURL" || logSilly "Key: KEY_PDN_NODE_TRACKERURL not applicable"
    docker_installToSystemYaml "$KEY_PDN_NODE_JOINKEY" "$FLAG_NOT_APPLICABLE" "$SYS_KEY_PDN_NODE_JOINKEY" || logSilly "Key: KEY_PDN_NODE_JOINKEY not applicable"
    docker_installToSystemYaml "$KEY_PDN_NODE_SELFGRPCADDRESS" "$FLAG_NOT_APPLICABLE" "$SYS_KEY_PDN_NODE_SELFGRPCADDRESS" || logSilly "Key: KEY_PDN_NODE_SELFGRPCADDRESS not applicable"
    docker_installToSystemYaml "$KEY_PDN_NODE_SELFHTTPADDRESS" "$FLAG_NOT_APPLICABLE" "$SYS_KEY_PDN_NODE_SELFHTTPADDRESS" || logSilly "Key: KEY_PDN_NODE_SELFHTTPADDRESS not applicable"
    docker_installToSystemYaml "$KEY_PDN_NODE_PORT" "$FLAG_NOT_APPLICABLE" "$SYS_KEY_PDN_NODE_PORT" || logSilly "Key: KEY_PDN_NODE_PORT not applicable"
    docker_installToSystemYaml "$KEY_PDN_NODE_HTTPPORT" "$FLAG_NOT_APPLICABLE" "$SYS_KEY_PDN_NODE_HTTPPORT" || logSilly "Key: KEY_PDN_NODE_HTTPPORT not applicable"
    docker_installToSystemYaml "$KEY_PDN_NODE_HTTPBOUNCERPORT" "$FLAG_NOT_APPLICABLE" "$SYS_KEY_PDN_NODE_HTTPBOUNCERPORT" || logSilly "Key: KEY_PDN_NODE_HTTPBOUNCERPORT not applicable"
...
docker_setUpPostgresCompose() {
    ...
    local postgresVer=$(echo "${JFROG_POSTGRES_13_4X_VERSION}" | tr '-' '.' )
...
docker_getUserInputs(){
    ...
    docker_getSystemValue "$KEY_PDN_NODE_TRACKERURL" "JF_PDN_NODE_TRACKERURL" "$INSTALLER_YAML"
    JF_PDN_NODE_TRACKERURL=${YAML_VALUE:-"$JF_PDN_NODE_TRACKERURL"}
    logDebug "Method: docker_getUserInputs. JF_PDN_NODE_TRACKERURL is now [$JF_PDN_NODE_TRACKERURL]"
